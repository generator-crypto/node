...
func GetQueryCmd(queryRoute string, cdc *codec.Codec) *cobra.Command {
	// Group generator queries under a subcommand
	generatorQueryCmd := &cobra.Command{
	...
	generatorQueryCmd.AddCommand(
		flags.GetCommands(
			GetCmdProfile(cdc),
		)...,
	)

	return generatorQueryCmd
}
...
func GetCmdProfile(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/generator/profile/%s/%s", address, coin), nil)
